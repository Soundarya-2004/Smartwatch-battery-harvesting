...
Include 2 helpful tips for improving energy harvesting and 1 caution if KE is too high. Keep it formal and concise."""
        ...
        try:
            ...
            if avg_ke < 2.0:
                tips = [
                    "Try to move more frequently throughout the day.",
                    "Short walks or light stretches can help boost energy harvesting."
                ]
                caution = "Low kinetic energy detected. Device may not charge effectively."
            elif avg_ke < 4.0:
                tips = [
                    "Maintain a steady pace during daily activities.",
                    "Consider using the device during light workouts or commutes."
                ]
                caution = "Moderate energy harvesting. Keep up the momentum!"
            elif avg_ke < 6.0:
                tips = [
                    "Great job! You're harvesting energy efficiently.",
                    "Keep your movement consistent for optimal battery gain."
                ]
                caution = "Watch for signs of overuse or fatigue."
            else:
                tips = [
                    "You're pushing the device to its limits. Consider short breaks.",
                    "Ensure the device remains cool during intense activity."
                ]
                caution = "High kinetic energy may cause overheating. Monitor device temperature."

            summary = (
                f"📊 Average Kinetic Energy: {avg_ke:.2f}\n"
                f"🔋 Current Battery Level: {self.battery:.1f}%\n\n"
                f"✅ Tips:\n- {tips[0]}\n- {tips[1]}\n\n"
                f"⚠️ Caution:\n- {caution}"
            )
            return summary
    ...
    def create_pdf_report(self, summary):
        safe_summary = summary.encode('latin-1', 'ignore').decode('latin-1')
        pdf = FPDF()
        pdf.add_page()
        pdf.set_font("Helvetica", 'B', 16)
        pdf.set_text_color(40, 40, 40)
        pdf.cell(0, 10, "SmartWatch Energy Harvesting Report", ln=True, align='C')
